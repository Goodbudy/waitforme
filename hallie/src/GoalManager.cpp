...
void GoalManager::registerBot(std::shared_ptr<TurtleBot> bot) {
    bots_[bot->getName()] = bot;
}

void GoalManager::sendGoal(const std::string& bot_name, double x, double y) {
    if (bots_.find(bot_name) != bots_.end()) {
        bots_[bot_name]->navigateTo(x, y);
    } else {
        RCLCPP_WARN(this->get_logger(), "TurtleBot '%s' not registered!", bot_name.c_str());
    }
}
