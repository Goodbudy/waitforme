...
//     lidar_subscriber_ = this->create_subscription<sensor_msgs::msg::LaserScan>(
//         "/scan", qos_profile, std::bind(&PeopleDetector::lidarCallback, this, std::placeholders::_1));
...
PeopleDetector::PeopleDetector() : Node("people_detector"), hog_() {
    ...
    // If the backend is V4L2, it should return 4 (CAP_V4L2)
...
void PeopleDetector::publishMarkers() {
    ...
    for (size_t i = 0; i < people_positions_.size(); ++i) {
        visualization_msgs::msg::Marker marker;
        marker.header.frame_id = "base_link"; // Set your frame ID here (e.g., "base_link")
        marker.header.stamp = this->now();
        marker.ns = "people";
        marker.id = i;
        marker.type = visualization_msgs::msg::Marker::CYLINDER;
...
void PeopleDetector::imageCallback(const sensor_msgs::msg::Image::SharedPtr msg) {
    ...
    // Convert to ROS2 image message and set frame_id
    std_msgs::msg::Header header = msg->header;
    header.frame_id = "camera_frame"; // Set your frame ID here
    auto processed_msg = cv_bridge::CvImage(header, "bgr8", image).toImageMsg();
    image_publisher_->publish(*processed_msg);
}
