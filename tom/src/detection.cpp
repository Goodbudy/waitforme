...
void ObjDetect::scanCallback(const sensor_msgs::msg::LaserScan::SharedPtr msg)
{
    ...
    {
        if (!std::isfinite(msg->ranges[i - 1]) || msg->ranges[i - 1] < msg->range_min || msg->ranges[i - 1] > msg->range_max)
            continue;
        ...
        {
            float prev_angle = msg->angle_min + (i - 1) * msg->angle_increment;
            float curr_angle = msg->angle_min + i * msg->angle_increment;

            previous.x = msg->ranges[i - 1] * std::cos(prev_angle);
            previous.y = msg->ranges[i - 1] * std::sin(prev_angle);

            current.x = msg->ranges[i] * std::cos(curr_angle);
            current.y = msg->ranges[i] * std::sin(curr_angle);
            ...
            if (dist < 0.075)
            {
                ...
                // Transform current point to global/map frame
                geometry_msgs::msg::Point global_point = localToGlobal(currentOdom, current);

                // Check if point is in free space (like original scanCallback)
                int mx = static_cast<int>((global_point.x - origin_x_) / resolution_);
                int my = static_cast<int>((global_point.y - origin_y_) / resolution_);
                int idx = my * width_ + mx;

                if (mx >= 0 && mx < width_ && my >= 0 && my < height_ &&
                    idx >= 0 && idx < static_cast<int>(grid_.size()) &&
                    grid_[idx] == 0)
                {
                    currentSegment.push_back(global_point);
                }
            }
            else
            {
        ...
        if (segStarted && !currentSegment.empty())
        {
            rclcpp::sleep_for(std::chrono::milliseconds(10));  // Optional delay for RViz
            if (!isThisAWall(currentSegment))
            {
                if (!isThisACorner(currentSegment))
                {
