...
public:
    ...
    {
        // Make our goal publisher transient_local so late‐joiners still see the first message
...
private:
    ...
    void publishGoal(double x, double y)
    {
        ...
        RCLCPP_INFO(get_logger(),
                    "Published goal (%.2f, %.2f) in state=%d",
                    x, y, static_cast<int>(state_));
    }
    ...
    void followPath()
    {
        ...
        auto &tgt = path_[current_target_idx_].pose;
        double dx = tgt.position.x - current_x_;
        double dy = tgt.position.y - current_y_;
        ...
        cmd.linear.x  = std::min(dist, 0.5) * 0.5;
        ...
        switch (state_) {
            ...
            case State::EXEC_GOAL_PATH:
                ...
                wait_timer_ = create_wall_timer(
                    ...
                    std::bind(&MovementLogic::onWaitComplete, this)
                );
                break;
